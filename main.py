...
def main():
    ...
    for ingredient in alg.find_undefined_ingredients(menu, data.ingredient_types):
        ...
        print(
            '    Ingredient(name=%r,purchase_unit=%s, category=u"fixme"),'
            % (ingredient.name, unt)
        )
    ...
    shopping_list = alg.subtract_from_shopping_list(
        shopping_list, data.prebought, data.ingredient_types
    )
    shopping_list = alg.subtract_from_shopping_list(
        shopping_list, data.buy_later, data.ingredient_types
    )
